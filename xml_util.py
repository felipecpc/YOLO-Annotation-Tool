...
def createXML(filename,folder, boxes, img_size):
...
def createXMLAnnotation(jpg_file, boxes, img_size, outFolder):
    filename = jpg_file.split("/")
    folder = filename[0]
    filename = filename[len(filename)-1]

    xml = createXML(filename,folder, boxes, img_size)
    outputFile = "%s/%s"%(outFolder,filename.replace(".jpg",".xml"))
    print(outputFile)
